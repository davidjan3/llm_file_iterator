...
```bash
python gemini_iterate.py "C:/path/**/*.ts" "./prompt.txt"
```

or

```bash
python claude_batch.py "C:/path/**/*.ts" "./prompt.txt"
```
...
Make sure to provide a Gemini or Anthropic API Key and Model ID via env and the following parameters via command line:
